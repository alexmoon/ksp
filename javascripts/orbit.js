...
(function() {
  var Orbit, acosh, clamp, cosh, crossProduct, gaussTimeOfFlight, sinh;
  ...
  Orbit.fromPositionAndVelocity = function(referenceBody, position, velocity, t) {
    ...
    if (eccentricity === 0) {
      ...
      if (nodeVector[1] < 0) {
        orbit.longitudeOfAscendingNode = 2 * Math.PI - orbit.longitudeOfAscendingNode;
      }
      orbit.argumentOfPeriapsis = Math.acos(numeric.dot(nodeVector, eccentricityVector) / (n * eccentricity));
      if (eccentricityVector[2] < 0) {
        orbit.argumentOfPeriapsis = 2 * Math.PI - orbit.argumentOfPeriapsis;
      }
    }
